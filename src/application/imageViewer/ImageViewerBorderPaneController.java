...
public class ImageViewerBorderPaneController {
	...
	private ObservableList<Integer> zoomRateItems = FXCollections.observableArrayList(1, 5, 10, 25, 50);
	 ...
	 * @param width
	 */
	private void setFocusPosition(double x, double y, double rate, double imageWidth, double imageHeight, int width) {
			x = Math.max(0, x);
			y = Math.max(0, y);
	...
	private void changeZoomRate(double rate) {
		imageOpt.filter(i -> mainController.getTKoolVersion().getWidth() < i.getWidth() * rate / 100)
			.ifPresent(i -> {
