...
/**
 * オプション設定画面のコントローラクラス。 更新した値がMainControllerに渡されるのは、OKボタンがクリックされた場合のみで、
...
public class OptionsStageController {
  ...
  private ObservableList<String> separatroItems =
      FXCollections.observableArrayList(Separators.getSeparatorsText());

  @FXML private ComboBox<String> numberingComboBox;
  private ObservableList<String> numberingItems =
      FXCollections.observableArrayList(Numberings.getNumberingsText());
  ...
  ObservableList<Integer> fontSizeItems =
      FXCollections.observableArrayList(8, 9, 10, 11, 12, 13, 14);
