...
public enum Separators {
  ...
  public static String[] getSeparatorsText() {
    ...
    IntStream.range(0, size).forEach(i -> separators[i] = values[i].getSeparatorText());
    return separators;
  }

  /**
   * 文字列の末尾が、Separatorsの定数の区切り線のいずれかとマッチした場合trueを返す。 いずれともマッチしなかった場合、つまり末尾に区切り線が存在しない場合はfalseを返す。
   *
   * @param target
   * @return
   */
  ...
  /**
   * Separators定数の名前と一致した場合、一致した定数を返す。 一致するものが存在しなかった場合はnullを返す。
   *
   * @param enumName
   * @return
   */
