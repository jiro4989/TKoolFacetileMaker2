...
/**
 * トリミングする際に利用される x,y座標、拡大率とグリッドペイン上の行列番号、 パネルに表示する元になっている画像のBufferedImageを保持する拡張クラス。
 *
 * @author jiro
 */
public class MyImageView extends ImageView {
  ...
  MyImageView(
      Image image, double aX, double aY, double aRate, String filePath, int aColumn, int aRow) {
    super(image);
    x = aX;
    y = aY;
    rate = aRate;
  ...
  /**
   * タイル上にプレビューされていたイメージを 実際にBuferedImageとして生成したパネル画像を返す。
   *
   * @param list
  ...
  public static BufferedImage makeTKoolFacetileImage(List<MyImageView> list, int width) {
    BufferedImage faceTileImage =
        new BufferedImage(width * 4, width * 2, BufferedImage.TYPE_INT_ARGB_PRE);
        ...
        .forEach(
            image -> {
              BufferedImage enlargedImage =
                  makeEnlargedImage(image.originalImage, image.rate / 100);
              BufferedImage trimmingImage =
                  enlargedImage.getSubimage((int) image.x, (int) image.y, width, width);
  ...
  public String toString() {
    return String.format(
        "x: %1.1f, y: %1.1f, rate: %1.1f, column: %d, row: %d, originalImage: %s",
        x, y, rate, column, row, originalImage);
  }
}
