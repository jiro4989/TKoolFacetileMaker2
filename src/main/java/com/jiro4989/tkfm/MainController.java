...
public class MainController {
	...
	private static final String[] KEYS = {
			"separator_switch", "separator", "numberings", "font_size",
			"tkool_version",
			"opened_file_name", "opened_dir_path",
			"saved_file_name", "saved_dir_path",
			"numbering_file_name"};
	private static final String[] INITIAL_VALUES = {
			"false", "UNDER_SCORE", "NUMBERING01", "12",
			"MV",
			"", ".",
			"", ".",
			"MyActor.png"};
	...
	private List<MyImageView> getPanelImages() {
		...
		List<MyImageView> images = imageList.stream()
	...
	/**
	 * ナンバリングしてファイルを保存する。
	 * ファイル名が存在しなかった場合は定義するためのダイアログを呼び出す。
	 */
	...
	/**
	 * ナンバリングして保存する際にファイル名を指定して保存する。
	 * ダイアログを表示してファイル名の入力を強制する。
	 */
	private void numberingSaveAsFile() {
		...
		name.ifPresent(f -> {
			numberingFileName = f;
			numberingFileName = numberingFileName.endsWith(".png") ? numberingFileName : numberingFileName + ".png";
	...
	/**
	 * プロパティファイルを書き出す。
	 * 呼び出し元はMainクラスで、ウィンドウを閉じるときに呼び出される。
	 */
